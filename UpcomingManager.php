...
//$query="SELECT AppitmentID, pet_name,serviceN,dateA,timeA,noteA,status FROM appointment WHERE status='yes'" ;
$query="SELECT * FROM  (appointment JOIN Pet ON Pet.id=appointment.PetID) WHERE status='yes'";
...
<html lang="en">
  ...
  <body>
    ...
    <section>
      <div class="row">
        <h1 class="head">Upcoming Appointment</h1>
      </div>
...
<?php foreach($row as $appoi){  ?>
  <div  class="col-md-4">
            ...
            <br><p><?php echo"note:" .$appoi['noteA'];   ?></p></p>
                        <div class="icons">
                       <br> <i class="fas fa-envelope"></i><a href="mailto:<?php echo"".$appoi['Owneremail'];   ?>">:contact owner</a>
                        <a onclick="return confirm('Are you sure that this Appointment is complete ?')" href="CompleteApp.php?id=<?php echo $appoi['AppitmentID']; ?>"><i class="fas fa-check"></i>Complete </a>
                        </div>
